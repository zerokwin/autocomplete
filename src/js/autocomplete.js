const AutoComplete = ((window) => {
    ...
    const autocomplete = function(id, url, opts) {
		this.id = id;
    ...
    autocomplete.prototype = {
		...
		render(data) {
			...
			for (var i = 0; i < len; i++) {
				data[i] = typeof data[i] === 'object' ? data[i].value : data[i];
	...
	function filter(kw) {
		...
		data = this.opts.src.filter(item => {
	...
	function changeListener(elem) {
		...
		addEventListener(that.elem, 'keyup', function() {
			let kw = that.elem.value.toLowerCase();

			that.index = -1;
			let data = filter.call(that, kw);
			that.render(data);
		});
	}
